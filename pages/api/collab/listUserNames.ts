  ...
  if (req.method == 'GET') {
    ...
    if (
      ...
      try {
        ...
        if (parsedPayload.userPrismaStripeCollab == true) {
          const userCompany = (
            jwtPayload as { userPrismaCompany: string }
          ).userPrismaCompany.toLowerCase();
          // .replace(/\s/g, "") to erase spaces
          const userCompanyWithoutSpaces =
            'list' + userCompany.replace(/\s/g, '') + 'userNames';
          ...
          if (cachedValue) {
            ...
            const extractedData = listUsers.map(
            ...
            redisSet(userCompanyWithoutSpaces, extractedData, 'userMngmt'); // no need to await this (and then impact timing) because we res something else: listUsers
