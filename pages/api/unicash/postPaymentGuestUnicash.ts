...
export default async function handler(
  ...
  if (req.method == 'POST') {
    try {
      ...
      const companyName = reqData.split('|')[1]?.trim() || '';
      ...
      try {
        ...
        const cityCookie = 'unicash-geolocation';
        ...
        const userCity = req.headers.cookie
          ?.split('; ')
          .find((row) => row.startsWith(cityCookie))
          ?.split('=')[1];
        const userCityValue = userCity?.valueOf();
        ...
        if (payMethod == 'cash') {
          ...
          const session = await stripe.checkout.sessions.create({
            success_url:
              ...
              encodeURIComponent(cipher),
            cancel_url:
              appContext.appUrl + '/unicash/' + companyName.toLowerCase(),
