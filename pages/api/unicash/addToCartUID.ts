...
export default async function handler(
  ...
  if (req.method == 'POST') {
    ...
    if (typeof req.body === 'string') {
      try {
        newProductToAdd = JSON.parse(req.body);
      } catch (error) {
        return res
          .status(400)
    ...
    try {
      ...
      try {
        ...
        if (cachedValue) {
          ...
          cartData.push(newProductToAdd);

          const updatedJsonString = JSON.stringify(cartData);

          await redisSet(keyCartUID, cartData, 'unicashCartMngmt');
          return res.status(200).json(updatedJsonString);
        } else {
          await redisSet(keyCartUID, newProductToAdd, 'unicashCartMngmt');
          return res.status(200).json(newProductToAdd);
        }
      } catch (error) {
