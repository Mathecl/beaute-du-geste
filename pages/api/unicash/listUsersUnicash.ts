  ...
  if (req.method == 'GET') {
    ...
    if (
      ...
      try {
        ...
        if (
          parsedPayload.userPrismaRole == 'admin' &&
          parsedPayload.userPrismaStripeCash == true
        ) {
          const userCompany = (jwtPayload as { userPrismaCompany: string })
            .userPrismaCompany;
          // .replace(/\s/g, "") to erase spaces
          const userCompanyWithoutSpaces =
            'list' + userCompany.replace(/\s/g, '') + 'UsersUnicash';
          ...
          if (cachedValue) {
            ...
            redisSet(userCompanyWithoutSpaces, listUsers, 'userMngmt'); // no need to await this (and then impact timing) because we res something else: listUsers
